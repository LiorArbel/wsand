...
class ECS {
  ...
  private checkES(entity: Entity, system: System): void {
    ...
    const existingSystem = this.systems.get(system);
    if(!existingSystem){
        throw "non existing system!";
    }
    let need = system.componentsRequired;
    if (container.hasAll(need)) {
      // should be in system
      existingSystem.add(entity); // no-op if in
    } else {
      // should not be in system
      existingSystem.delete(entity); // no-op if out
    }
  }
}
